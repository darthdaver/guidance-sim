 ...
 * @param {Object} config.routes Mapbox Directions API response. May be `v4` or `v5`.
 ...
 */
module.exports = run;
function run (config, frequency, speedInterval) {
  var version = util.version(config);
  return simulate(config, frequency, speedInterval, version);
}
...
function simulate(config, frequency, speedInterval, version) {
  var response = new events();
  var maneuvers = getManeuvers(config, version); // retrieve maneuvers specified in configuration file
  var route = getRoute(config.routes, { 'spacing': 'acceldecel' }); // retrieve a GeoJSON of the route
  ...
  var speed = util.speed(speedInterval, frequency); // determine the start speed

  var locator = new Locator(config.routes); // initilize the Locator
...
function setMap(interval, speed, options) {
  ...
  if (step == null) {
    ...
    step['speed'] = util.distanceConvert(step['speed'], 'km'); // convert speed to miles per hour
...
function modifyParams (config, version, step, maneuvers) {
  ...
  for (key in maneuvers) {
    if (maneuvers.hasOwnProperty(key)) {
      ...
      var distance = turfDistance(maneuver, turfPoint(step['coords']), 'kilometers');
...
function zoomBySpeed (config, step) {
  ...
  var response = 60 - zoomDiff; // determine new zoom level
  return response;
}
